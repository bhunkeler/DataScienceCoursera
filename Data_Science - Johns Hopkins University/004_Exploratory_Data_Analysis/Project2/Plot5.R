# ========================================================================================================================================
# Description:   Course Project 2 / Plot5.R
...
# Some comment here. Question 5 is not very precise. Therefore I'm not sure what qualifies to be a vehicle. 
# Does "highway vehicle" as well as "off-highway vehicle" qualify as vehicle. 
# In the version below I retrieved the data based on the "SCC.Level.Two" column. This is a bit more work.
# 
# Remark:
# Both versions work, even though I decided to go the easy route and handed in the version above. 
# A more detailed desciption can be found in the markdown 

if (SCC.Level.2) {
    pm25_Baltimore_ <- subset(SCC, fips == '24510')
    dataSCC_ <- ddply(pm25_Baltimore_, .(year), summarise, Emissions = round(sum(Emissions), 2))

    # remove not required columns 
    NEI <- NEI[, c('SCC', 'SCC.Level.Two')]

    # convert all fields to lower case 
    NEI$SCC.Level.Two <- lapply(NEI$SCC.Level.Two, function(x) tolower(as.character(x)))

    # define regex pattern
    pattern <- '*highway vehicle[a-z -]*'

    # subset and retrieve only comb / coal data based on regex pattern
    vehicle <- subset(NEI, grepl(pattern, NEI$SCC.Level.Two))

    # evaluate which code exists in both data sets
    exist_in_both <- ifelse(pm25_Baltimore_$SCC %in% vehicle$SCC, TRUE, FALSE)
    pm25_Baltimore_ <- subset(pm25_Baltimore_, exist_in_both)

    # calculate total amount of emissions per year and SCC 
    pm25_Baltimore_ <- ddply(pm25_Baltimore_, .(year), summarise, Emissions = round(sum(Emissions), 2))

    # ========================================================================================================================================
    # Create and plot graph
    # ========================================================================================================================================

    png(filename = "plot4_.png", width = 600, height = 600, units = "px", bg = "white")
    ...
    # add a line between points
    lines(pm25_Baltimore_$year, pm25_Baltimore_$Emissions, type = "l", lwd = 2)
    axis(1, c(1999, 2002, 2005, 2008))

    # print values for each point in graph
    text(pm25_Baltimore_$year, pm25_Baltimore_$Emissions, pm25_Baltimore_$Emissions, cex = 1.0, pos = 4, col = "black")
